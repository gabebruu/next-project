...
export default function ComponenteComEventos() {
  ...
  return (
    <div className="bg-red-100 border border-red-400 text-pink-700 px-4 py-3 rounded mb-4">
      <h2 className="text-xl font-semibold">Eventos e Cliques (event handlers)</h2>
      
      {/* BOTÃO 1: Função normal */}
      <button onClick={handleClick} className="bg-blue-300 hover:bg-purple-300 text-black font-bold py-1 px-3 rounded mr-2 mb-2">
        Função Normal
      </button>
      
      {/* BOTÃO 2: Arrow function */}
      <button onClick={handleClickArrow} className="bg-purple-400 hover:bg-amber-700 text-black font-bold py-1 px-3 rounded mr-2 mb-2">
        Arrow Function
      </button>
      
      {/* BOTÃO 3: Com parâmetro (usa arrow function inline) */}
      <button  onClick={() => handleClickComParametro("Olá do botão 3!")} className="bg-red-500 hover:bg-red-700 text-black font-bold py-1 px-3 rounded mr-2 mb-2">
        Com Parâmetro
      </button>
